...
import coverImg from '@/assets/images/report/capitalMultiplication/cover.png';
...
const CapitalMultiplicationReportComponent = ({ number, target }: GoalReportType) => {
    ...
    return (
        <ReportLayout title='Накопить капитал' number={`${number}`} isTarget months={target.months} riskScoreTotal={riskScoreTotal}>
                ...
                <div className={styleG.top}>
                    <ul className={styleG.top__list}>
                        <li className={styleG.top__list_item}>
                            ...
                            <span className={styleG.top__list_item_text__blue}>{(target.cost || 0).toLocaleString('ru-RU', { style: 'currency', currency: target.currency || 'RUB', maximumFractionDigits: 0 }).replace(/^(\D+)/, '$1 ').replace('CN', '')}</span>
                        </li>
                        <li className={styleG.top__list_item}>
                            <span className={styleG.top__list_item_text}>{'Стоимость цели\nс учётом инфляции'}</span>
                            <span className={styleG.top__list_item_text__blue}>{(target.costWithInflation || 0).toLocaleString('ru-RU', { style: 'currency', currency: target.currency || 'RUB', maximumFractionDigits: 0 }).replace(/^(\D+)/, '$1 ').replace('CN', '')}</span>
                        </li>
                        <li className={styleG.top__list_item}>
                            <span className={styleG.top__list_item_text}>Первоначальный капитал</span>
                            <span className={styleG.top__list_item_text__blue}>{(target.initialCapital || 0).toLocaleString('ru-RU', { style: 'currency', currency: target.currency || 'RUB', maximumFractionDigits: 0 }).replace(/^(\D+)/, '$1 ').replace('CN', '')}</span>
                        </li>
                        <li className={styleG.top__list_item}>
                            <span className={styleG.top__list_item_text}>{'Рекомендованное\nежемесячное пополнение'}</span>
                            <span className={styleG.top__list_item_text__blue}>{(target.initialReplenishment || 0).toLocaleString('ru-RU', { style: 'currency', currency: target.currency || 'RUB', maximumFractionDigits: 0 }).replace(/^(\D+)/, '$1 ').replace('CN', '')}</span>
                        </li>
                    </ul>
                </div>
                <div className={styleG.top}>
                ...
                <div className={styleG.offer_container}>
                    <div className={styleG.offer_item}>
                        ...
                        <table className={styleG.offer_item__table}>
                            <thead className={styleG.offer_item__table__head}><tr className={styleG.offer_item__table__head_row}><td>Продукт</td><td>Сумма</td></tr></thead>
                            <tbody className={styleG.offer_item__table__body}>
                                {portfolioInitial.map((tool, i) => (<tr className={styleG.offer_item__table__body_row} key={i}><td>{tool.title}</td><td>{Number(tool.sum).toLocaleString('ru-RU', { style: 'currency', currency: tool.currency || 'RUB', maximumFractionDigits: 0 }).replace(/^(\D+)/, '$1 ').replace('CN', '')}</td></tr>))}
                            </tbody>
                        </table>
                    </div>
                </div>
                <div className={styleG.offer_container}>
                    <div className={styleG.offer_item}>
                        ...
                        <table className={styleG.offer_item__table}>
                            <thead className={styleG.offer_item__table__head}><tr className={styleG.offer_item__table__head_row}><td>Продукт</td><td>Сумма</td></tr></thead>
                            <tbody className={styleG.offer_item__table__body}>
                                {portfolioReplenishment.map((tool, i) => (<tr className={styleG.offer_item__table__body_row} key={i}><td>{tool.title}</td><td>{Number(tool.sum).toLocaleString('ru-RU', { style: 'currency', currency: tool.currency || 'RUB', maximumFractionDigits: 0 }).replace(/^(\D+)/, '$1 ').replace('CN', '')}</td></tr>))}
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <div className={styleG.footer}>
                {portfolioInitial.length ? (
                    <span className={styleG.footer_title}>{'Доходность стратегии*\n'}<span className={styleG.footer_title_wrap}>{`${(((portfolioInitial.reduce((a, b) => ((b.sum * (b.percent || 0)) / 100) + a, 0) / portfolioInitial.reduce((a, b) => a + b.sum, 0)) || 0) * 100).toFixed(2)}%`}</span></span>
                ) : null}
...
export default CapitalMultiplicationReportComponent;
