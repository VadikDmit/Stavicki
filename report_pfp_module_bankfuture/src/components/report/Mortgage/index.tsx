...
import styleM from './style.module.scss';
...
const MortgageReportComponent = ({ number, target }: GoalReportType) => {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < target.months; i++) {
            ...
            dayNow.setUTCMonth(i + dayNow.getMonth());
            if (i !== 0 && target.mortgageRate) {
                balance -= ((target.initialReplenishment || 0) - (balance * target.mortgageRate / 1200));
            }
            chartData.push({ value: Math.floor(balance * -1), date: dayNow.toLocaleDateString('ru-RU') });
        }
    ...
    return (
        <ReportLayout title='Ипотека на квартиру' number={`${number}`} isTarget>
                ...
                <div className={style.top_container}>
                    ...
                    <div className={`${style.top} ${style.top__morgage}`}>
                        <nav className={style.top__list}>
                            <ul>
                                <li className={style.top__list_item}>
                                    <span className={style.top__list_item_text}>Стоимость цели</span>
                                    <span className={style.top__list_item_text__blue}>{fmt(target.cost || 0)}</span>
                                </li>
                                <li className={style.top__list_item}>
                                    <span className={style.top__list_item_text}>Первоначальный взнос</span>
                                    <span className={style.top__list_item_text__blue}>{fmt(target.initialCapital || 0)}</span>
                                </li>
                                <li className={style.top__list_item}>
                                    <span className={style.top__list_item_text}>Сумма кредита</span>
                                    <span className={style.top__list_item_text__blue}>{fmt((target.cost || 0) - (target.initialCapital || 0))}</span>
                                </li>
                                <li className={style.top__list_item}>
                                    <span className={style.top__list_item_text}>Срок кредита</span>
                                    <span className={style.top__list_item_text__blue}>{`${target.months / 12} лет`}</span>
                                </li>
                                <li className={style.top__list_item}>
                                    <span className={style.top__list_item_text}>Ставка</span>
                                    <span className={style.top__list_item_text__blue}>{`${target.mortgageRate || 0}%`}</span>
                                </li>
                                <li className={style.top__list_item}>
                                    <span className={style.top__list_item_text}>Ежемесячный платеж</span>
                                    <span className={style.top__list_item_text__blue}>{fmt(target.initialReplenishment || 0)}</span>
                                </li>
                            </ul>
                        </nav>
                    </div>
            ...
            <div className={style.footer} style={{ marginBlockStart: 30, alignItems: 'flex-start' }}>
                <div className={style.footer_chart}>
            ...
            <p className={style.footer_text}>
                *- расчет страховых премий сделан на основе средних значений. Вы можете узнать стоимость страхования у аккредитованных СК
            </p>
        </ReportLayout>
    );
};

export default MortgageReportComponent;
