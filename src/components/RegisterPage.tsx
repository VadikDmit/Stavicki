...
const RegisterPage: React.FC<RegisterPageProps> = ({ onRegisterSuccess, onSwitchToLogin }) => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // В консоль для отладки (F12 → Console)
            console.error('Register error:', err?.response?.status, err?.response?.data, err?.message);

            const status = err.response?.status;
            const data = err.response?.data;
            let msg = data?.message || data?.error || err.message || 'Ошибка при регистрации';
            if (typeof data === 'object' && Object.keys(data).length > 0 && !data.message && !data.error) {
